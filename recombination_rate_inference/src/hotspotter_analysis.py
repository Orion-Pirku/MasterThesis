...
class GCAnalyzer:
    __slots__ = ["gc_windows", "genome_file"]
    def __init__(self, genome_file_name: str):
        self.genome_file: Path = Path(genome_file_name).expanduser()
        self.gc_windows: BedTool = None
        
    def compute_gc_content(self, bed_object: BedTool) -> BedTool:
        genome_fna_file = Path(self.genome_file).expanduser()
        bed = (
            BedTool.from_dataframe(bed_object)
            if isinstance(bed_object, pd.DataFrame)
            else bed_object
        )
        self.gc_windows = bed.nucleotide_content(fi=genome_fna_file)
        return self.gc_windows
    
    def compute_genome_wide_correlation(
        self, 
        bed_object: BedTool) -> np.ndarray:
        
        bed_object_score_array: np.ndarray = bed_object.to_dataframe().iloc[:, 2].to_numpy()
        gc_windows_array: np.ndarray = self.gc_windows.to_dataframe().iloc[:, 2].to_numpy()
