...
for file in vcf/sorted_*; do
	for i in 10000 100000 1000000; do
		vcftools --gzvcf "$file" --SNPdensity "${i}" --out "vcf_stats/snp_density_${i}_${file##*/}"
		vcftools --gzvcf "$file" --window-pi "${i}" --out "vcf_stats/window_pi_${i}_${file##*/}"
		vcftools --gzvcf "$file" --TajimaD "${i}" --out "vcf_stats/tajimaD_${i}_${file##*/}"
	done
done
echo "done"
