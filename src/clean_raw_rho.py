...
from hotspotter.transform import make_windows, sort_windows, concatenate_windows
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--concatenate",
        action="store_true",
        help="Concatenate all outputs into a single file.",
    )
...
def main() -> None:
    ...
    if args.window_size and args.window_size > 0:
        recombination_windows_list: list[pd.DataFrame] = [
            make_windows(df, args.window_size, args.effective_pop_size)
            for df in sorted_raw_rec_data
        ]
    else:
        recombination_windows_list = sorted_raw_rec_data
    if args.concatenate is True:
        concatenated_recombination_windows: pd.DataFrame = concatenate_windows(
            recombination_windows_list
        )
        concatenated_recombination_windows.to_csv(
            args.output_file, sep="\t", header=False, index=False
        )
    else:
        for df in recombination_windows_list:
            chr = df.iloc[0, 0]
            df.to_csv(f"{chr}_{args.output_file}", sep="\t", header=True, index=False)
